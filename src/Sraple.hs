...
url =
  "https://seekingalpha.com/article/4132283-jabils-jbl-ceo-mark-mondello-q1-2018-results-earnings-call-transcript?all=true&find=conference%2520call"

e content = scrapeURL "mp.weixin.qq.com/s/4_osNdJ8FlVuzSKf1MGXrg" content

m content =
  scrapeURL "https://www.linkedin.com/in/chenyu-xiong-0abb6a91/" content
...
main = do
  ...
  print html
  where printError = putStrLn "Failed"
...
categorise (Contents q) [] a =
  reverse ([((head $ reverse a) ++ [Contents q])] ++ (tail $ reverse a))
...
b =
  [ Node Nothing [Node Nothing [Node (Just 1) [], Node (Just 6) []]]
  , Node Nothing [Node (Just 3) []]
  ]
...
getTitle url =
  (scrapeURLWithConfig (Config utf8Decoder Nothing) url $ titleParser)
